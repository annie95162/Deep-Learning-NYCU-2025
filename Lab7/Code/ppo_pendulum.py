...
class Actor(nn.Module):
    def __init__(
        ...
        self.hidden1 = init_layer_uniform(nn.Linear(in_dim, 128))
        self.hidden2 = init_layer_uniform(nn.Linear(128, 128))
...
class Critic(nn.Module):
    def __init__(self, in_dim: int):
        ...
        self.hidden1 = init_layer_uniform(nn.Linear(in_dim, 128))
        self.hidden2 = init_layer_uniform(nn.Linear(128, 128))
        self.value_layer = init_layer_uniform(nn.Linear(128, 1))
            ...
            for _ in range(self.rollout_len):
                ...
                if done[0][0]:
                    ...
                    if len(recent_scores) > 20:
                        recent_scores.pop(0)
                    if len(recent_scores) == 20 and all(r > -150 for r in recent_scores) and (save <= 20):
                        model_name = f"LAB7_task2_ppo_ep{episode_count}_step{self.total_step}.pt"
                        print(f"\n Saving model at episode {episode_count}: {model_name}")
