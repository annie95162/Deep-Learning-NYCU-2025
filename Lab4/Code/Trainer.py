...
from Plotting import plot_loss_curve, plot_teacher_forcing_ratio, plot_psnr_per_frame
...
class VAE_Model(nn.Module):
    ...
    def training_stage(self):
        ...
        for i in range(self.args.num_epoch):
            ...
            teacher_forcing_ratios.append(self.tfr)
        ...
        plot_teacher_forcing_ratio(teacher_forcing_ratios, range(self.args.num_epoch), 
                               kl_type=self.args.kl_anneal_type, 
                               cycle=self.args.kl_anneal_cycle, 
                               ratio=self.args.kl_anneal_ratio)
    ...
    def eval(self):
        ...
        for (img, label) in (pbar := tqdm(val_loader, ncols=120)):
            img = img.to(self.args.device)
            label = label.to(self.args.device)
            loss, psnr_list = self.val_one_step(img, label)
            
            #val_losses.append(loss.item())
            val_losses.append(loss.item())
            psnr_values.extend(psnr_list)
            self.tqdm_bar('val', pbar, loss.detach().cpu(), lr=self.scheduler.get_last_lr()[0])
        
        # print("size: ", len(psnr_values))    
        
        if (self.current_epoch + 1) == self.args.num_epoch:    
            plot_loss_curve(val_losses, range(len(val_losses)),
                    kl_type=self.args.kl_anneal_type, cycle=self.args.kl_anneal_cycle, ratio=self.args.kl_anneal_ratio)
        
            plot_psnr_per_frame(psnr_values, range(len(psnr_values)),
                        kl_type=self.args.kl_anneal_type, 
                        cycle=self.args.kl_anneal_cycle, 
                        ratio=self.args.kl_anneal_ratio)
