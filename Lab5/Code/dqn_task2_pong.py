...
class DQNAgent:
    def __init__(self, env_name="ALE/Pong-v5", args=None):
        ...
        self.memory = PrioritizedReplayBuffer(args.memory_size) # 新增
    ...
    def run(self, episodes=10000):
        for ep in range(episodes):
            ...
            while not done and step_count < self.max_episode_steps:
                ...
                self.memory.add((state, action, reward, next_state, done)) #修改
    ...
    def evaluate(self):
        ...
        total_reward = 0
        while not done:
            state_tensor = torch.from_numpy(np.array(state)).float().unsqueeze(0).to(self.device) / 255.0 # 新增 /255
            with torch.no_grad():
    ...
    def train(self):
        ...
        if self.epsilon > self.epsilon_min:
            ...
            self.epsilon = max(self.epsilon, self.epsilon_min) #新增
        self.train_count += 1
...
if __name__ == "__main__":
    ...
    agent = DQNAgent(env_name="ALE/Pong-v5", args=args) # 新增
    agent.run()
